...
const JobDetail = () => {
  ...
  // Sử dụng hook để xử lý logic ứng tuyển
  const { 
    applyForJob, 
  ...
  useEffect(() => {
    if (jobId) {
      ...
      console.error("Không tìm thấy ID việc làm trong URL");
      setLoading(false);
    }
  }, [jobId]);
  ...
  const fetchJobDetails = async (id) => {
    setLoading(true);
    try {
      // Trong thực tế, sử dụng API call để lấy chi tiết công việc
  ...
  const handleSubmitApplication = async (formData) => {
    try {
      // Gửi đơn ứng tuyển sử dụng hook
      const result = await applyForJob();
      
      if (result.success) {
        setShowApplyModal(false);
        
        // Làm mới trạng thái ứng tuyển
        fetchJobApplications();
      }
    } catch (error) {
